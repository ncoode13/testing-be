...
class AttendanceController extends ApiController
{
        ...
        try {
            return DB::transaction(function () use ($request, $imageService, $user, $currentUserId) {
                ...
                if ($request->attendance_type == 'check_in') {
                    ...
                    // --- TAMBAHAN UNTUK OVERNIGHT SHIFT ---
                    // Cek apakah ada absen masuk kemarin yang belum check out
                    if (!$attendance) {
                        $yesterday = Carbon::parse($date)->subDay()->toDateString();
                        $yesterdayAttendance = Attendance::with(['shift', 'logs'])
                            ->where('user_id', $currentUserId)
                            ->where('date', $yesterday)
                            ->first();

                        if ($yesterdayAttendance && $yesterdayAttendance->logs->where('attendance_type', 'check_out')->isEmpty()) {
                            $attendance = $yesterdayAttendance;
                            $responseMsg = 'Berhasil melakukan Check-out (Shift Lintas Hari).';
                        }
                    }
                    ...
                    if (!$attendance) {
                        ...
                        // Jangan timpa responseMsg jika sudah di-set oleh Shift Lintas Hari
                    ...
                    if ($shift) {
                        ...
                        $shiftEndTime = Carbon::parse($attendanceDate . ' ' . $shift->end_time);

                        // Jika shift overnight (end_time < start_time), maka jam pulang adalah hari berikutnya
                        if (Carbon::parse($shift->end_time)->lessThan(Carbon::parse($shift->start_time))) {
                            $shiftEndTime->addDay();
                        }

                        // --- BLOKIR LATE CHECK-OUT (Lebih dari 4 jam setelah shift berakhir) ---
                        $deadlineOut = $shiftEndTime->copy()->addHours(4);
                        if ($nowObject->greaterThan($deadlineOut)) {
                            throw new \Exception('Batas waktu Check-out sudah habis (lebih dari 4 jam setelah shift berakhir). Silakan ajukan Form Koreksi Absensi.');
                        }
    ...
    public function dashboardSummary(Request $request)
    {
        try {
            ...
            $totalNoClockOut = $attendances->whereIn('status', [1, 3, 6, 8, 9])
                ->filter(fn($att) => $att->logs->where('attendance_type', 'check_out')->isEmpty())
                ->count();

            $totalNoClockIn = $attendances->whereIn('status', [1, 3, 6, 8, 9])
    ...
    public function getMonthlyHistory(Request $request)
    {
        ...
        // 2. Override dengan ShiftSchedule dari Matrix (Berlaku untuk semua, termasuk Office)
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            ...
            if ($attendance) {
                ...
                if ($statusId === \App\Models\Attendance::STATUS_PRESENT) {
                    $statusBadge = 'present';
                    $badgeLabel = 'Hadir';
                    $badgeColor = 'green';
                } elseif ($statusId === \App\Models\Attendance::STATUS_LATE) {
                    $statusBadge = 'late';
                    $badgeLabel = 'Terlambat';
                    $badgeColor = 'orange';
                } elseif ($statusId === \App\Models\Attendance::STATUS_EARLY_OUT) {
                    $statusBadge = 'early_leave';
                    $badgeLabel = 'Pulang Awal';
                    $badgeColor = 'gray';
                } elseif ($statusId === \App\Models\Attendance::STATUS_PENDING) {
                    $statusBadge = 'pending';
                    $badgeLabel = 'Menunggu Konfirmasi';
                    $badgeColor = 'orange';
                } elseif ($statusId === \App\Models\Attendance::STATUS_REJECTED) {
                    $statusBadge = 'rejected';
                    $badgeLabel = 'Ditolak';
                    $badgeColor = 'red';
                } elseif ($statusId === \App\Models\Attendance::STATUS_LEAVE) {
                    $statusBadge = 'leave';
                    $badgeLabel = 'Cuti';
                    $badgeColor = 'blue';
                } elseif ($statusId === \App\Models\Attendance::STATUS_NCI) {
                    $statusBadge = 'nci';
                    $badgeLabel = 'No Clock In';
                    $badgeColor = 'orange';
                } elseif ($statusId === \App\Models\Attendance::STATUS_NCO) {
                    $statusBadge = 'nco';
                    $badgeLabel = 'No Clock Out';
                    $badgeColor = 'orange';
                } else {
                    $statusBadge = 'absent';
                    $badgeLabel = 'Alpa';
    ...
    public function getRewardData(Request $request)
    {
        ...
        // ==== EXCLUSION WINNERS ====
        $winnerUserIds = \App\Models\RewardWinner::where('year', $year)
            ->pluck('user_id')
            ->toArray();
        ...
        foreach ($users as $user) {
            // Jika sudah menang tahun ini, exclude dari kandidat
            if (in_array($user->id, $winnerUserIds)) {
                continue;
            }
    ...
    /**
     * Menyimpan daftar pemenang penghargaan
     */
    public function saveRewardWinners(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        try {
            ...
            foreach ($request->user_ids as $userId) {
                // Hindari duplikasi jika sudah ada di tahun yang sama
                $exists = \App\Models\RewardWinner::where('user_id', $userId)
                    ->where('year', $request->year)
                    ->exists();

                if (!$exists) {
                    \App\Models\RewardWinner::create([
                        'user_id' => $userId,
                        'year'    => $request->year,
                        'month'   => $request->month,
                    ]);
            ...
            DB::commit();
            return $this->respondSuccess(null, "Berhasil menyimpan $savedCount pemenang penghargaan.");
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->respondError('Gagal menyimpan pemenang: ' . $e->getMessage(), 500);
        }
    }

    /**
     * Mengambil daftar pemenang penghargaan tahun ini
     */
    public function getRewardWinners(Request $request)
    {
        $year = $request->query('year', now()->year);

        $winners = \App\Models\RewardWinner::with(['user.employee.department', 'user.employee.position'])
            ->where('year', $year)
            ->get();

        $formattedWinners = $winners->map(function ($winner) {
            return [
                'id' => $winner->id,
                'user_id' => $winner->user_id,
                'name' => $winner->user->name ?? 'Unknown',
                'department' => $winner->user->employee->department->name ?? '-',
                'position' => $winner->user->employee->position->name ?? '-',
                'year' => $winner->year,
                'month' => $winner->month,
            ];
        });

        return $this->respondSuccess($formattedWinners, "Data pemenang tahun $year berhasil diambil");
    }

    /**
     * Menghapus pemenang penghargaan (Batal Centang)
     */
    public function deleteRewardWinner($id)
    {
        try {
            $winner = \App\Models\RewardWinner::findOrFail($id);
            $winner->delete();

            return $this->respondSuccess(null, 'Data pemenang berhasil dibatalkan/dihapus.');
        } catch (\Exception $e) {
            return $this->respondError('Gagal menghapus data: ' . $e->getMessage(), 500);
        }
    }
}
