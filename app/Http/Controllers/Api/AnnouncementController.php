...
class AnnouncementController extends ApiController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            try {
                // Query user yang memiliki profil employee
                $query = User::whereHas('employee');

                // Jika berdasarkan kriteria, filter query user-nya
                if (!$payload['is_publish_to_all'] && !empty($payload['target_criteria'])) {
                    $criteria = $payload['target_criteria'];
                    $query->whereHas('employee', function ($q) use ($criteria) {
                        if (!empty($criteria['branches'])) $q->whereIn('branch_id', $criteria['branches']);
                        if (!empty($criteria['departments'])) $q->whereIn('department_id', $criteria['departments']);
                        if (!empty($criteria['positions'])) $q->whereIn('position_id', $criteria['positions']);
                        if (!empty($criteria['job_levels'])) $q->whereIn('job_level_id', $criteria['job_levels']);
                    });
                }
                ...
                if ($targetUsers->isNotEmpty()) {
                    FacadesNotification::send(
                        ...
                        new AnnouncementNotification(
                            ...
                            "Terdapat informasi terbaru untuk Anda. Silakan cek detailnya.",
                            $announcement->id,
                            Auth::user()->name
                        )
                    );
                ...
                \Log::error("Notifikasi Pengumuman Gagal: " . $e->getMessage());
            }
            ...
            try {
                ...
                if ($targetUsers->isNotEmpty()) {
                    FacadesNotification::send(
                        $targetUsers, 
                        new AnnouncementNotification(
                            "Update Pengumuman: " . $announcement->title,
                            "Terdapat perubahan informasi pada pengumuman. Silakan cek kembali.",
                            $announcement->id,
                            Auth::user()->name
                        )
                    );
                ...
                \Log::error("Notifikasi Update Pengumuman Gagal: " . $e->getMessage());
            }
